...
async fn main() -> std::io::Result<()> {
    let binding_address = std::env::var("BINDING_ADDRESS").map_err(|_| {
        std::io::Error::new(std::io::ErrorKind::Other, "BINDING_ADDRESS is not set")
    })?;
    ...
    let binding_port: u16 = std::env::var("BINDING_PORT")
        ...
        .parse()
        .map_err(|_| {
            std::io::Error::new(std::io::ErrorKind::Other, "Failed to parse BINDING_PORT")
        })?;
